...
# Tyrant Stats (applied to Player on select) -------
...
# RMB: Thunder Clap ---------------------------------
const THUNDER_CLAP_DAMAGE = 45.0
const THUNDER_CLAP_RANGE = 8.0
const THUNDER_CLAP_ANGLE = 60.0
const THUNDER_CLAP_HP_COST = 0.0
const THUNDER_CLAP_PUSH = 50.0
var thunder_clap_timer = 0.0
const THUNDER_CLAP_FIRE_RATE = 3.0

# Q: Bloodlust --------------------------------------
const BLOODLUST_DAMAGE = 0.0
const BLOODLUST_RADIUS = 10.0
const BLOODLUST_HP_COST = 0.0
const BLOODLUST_COOLDOWN = 15.0
const BLOODLUST_SLOW = 0.15
const BLOODLUST_SLOW_DURATION = 3.0
const BLOODLUST_LIFESTEAL_DURATION = 3.0
const BLOODLUST_LIFESTEAL_PCT = 0.8
var bloodlust_cooldown = 0.0
var lifesteal_timer = 0.0

# E: Incite / Inner Strength -----------------------
const INCITE_HP_COST = 0.20
const INCITE_INFLUENCE_FLAT = 20.0
const INCITE_INFLUENCE_PCT = 0.20
const INCITE_COOLDOWN = 1.5
var incite_cooldown = 0.0
...
func process_class(delta: float) -> void:
	# Update cooldowns -------------------------------
	if thunder_clap_timer > 0:
		thunder_clap_timer -= delta
	if bloodlust_cooldown > 0:
		bloodlust_cooldown -= delta
	if incite_cooldown > 0:
		incite_cooldown -= delta
	...
	if is_charging_inner_strength:
		...
		if inner_strength_timer <= 0:
			...
			print("Tyrant: Inner Strength unleashed")
	...
	if influence >= MAX_INFLUENCE and not is_consumed:
		...
		print("Tyrant: Consumed — 5s active window")
	...
	if is_consumed_active:
		...
		if consumed_timer <= 0:
			...
			print("Tyrant: Consumed decay phase — effects off, draining to ", CONSUMED_DECAY_TARGET)
	...
	if is_consumed_decaying:
		...
		if influence <= CONSUMED_DECAY_TARGET:
			is_consumed = false
			is_consumed_decaying = false
			print("Tyrant: Consumed fully ended")
	...
	skill_label.text = "Skill: Ready" if bloodlust_cooldown <= 0 else "Skill: " + str(snapped(bloodlust_cooldown, 0.1)) + "s"
	if is_charging_inner_strength:
		...
		ult_label.text = "Ult: Ready" if incite_cooldown <= 0 else "Ult: " + str(snapped(incite_cooldown, 0.1)) + "s"
...
func melee_attack() -> void:
	...
	for body in player.get_tree().get_nodes_in_group("enemies"):
		...
		# Lifesteal during Bloodlust buff
		if lifesteal_timer > 0:
			var heal = (player.max_hp - player.hp) * BLOODLUST_LIFESTEAL_PCT
...
func ranged_attack() -> void:
	if thunder_clap_timer > 0:
		return
	var fire_rate = _get_cooldown(THUNDER_CLAP_FIRE_RATE)
	player.ranged_animation(fire_rate)
	thunder_clap_timer = fire_rate
	var space_state = player.get_world_3d().direct_space_state
	...
	for body in player.get_tree().get_nodes_in_group("enemies"):
		...
		var distance = to_enemy.length()
		if distance > THUNDER_CLAP_RANGE:
			continue
		var angle = rad_to_deg(to_enemy.normalized().angle_to(cam_forward))
		if angle > THUNDER_CLAP_ANGLE / 2:
			continue
		var ray_query = PhysicsRayQueryParameters3D.create(cam_pos, body.global_position, 2)
		var result = space_state.intersect_ray(ray_query)
		if result and result.collider != body:
			continue
		body.take_damage(_get_damage(THUNDER_CLAP_DAMAGE))
		var push_dir = (body.global_position - player.global_position).normalized()
		push_dir.y = 0.0
		body.set("knockback_velocity", push_dir * THUNDER_CLAP_PUSH)
		body.set("stunned_timer", 0.8)
		if lifesteal_timer > 0:
			var heal = (player.max_hp - player.hp) * BLOODLUST_LIFESTEAL_PCT
			player.hp = min(player.hp + heal, player.max_hp)
...
func skill() -> void:
	if bloodlust_cooldown > 0:
		return
	bloodlust_cooldown = _get_cooldown(BLOODLUST_COOLDOWN)
	var slowed: Array[Node] = []
	var stored_speeds: Dictionary = {}
	for body in player.get_tree().get_nodes_in_group("enemies"):
		if not is_instance_valid(body):
			continue
		if player.global_position.distance_to(body.global_position) <= BLOODLUST_RADIUS:
			var s = body.get("speed_multiplier")
			if s != null:
				body.set("speed_multiplier", s * (1.0 - BLOODLUST_SLOW))
				slowed.append(body)
				stored_speeds[body] = s
	lifesteal_timer = BLOODLUST_LIFESTEAL_DURATION
	combat_timer = COMBAT_TIMEOUT
	_reset_passive_timer()
	await get_tree().create_timer(BLOODLUST_SLOW_DURATION).timeout
	for body in slowed:
...
func ult() -> void:
	...
	if influence >= BERSERK_THRESHOLD and _can_gain_influence():
		...
		print("Tyrant: Inner Strength charging — ", INNER_STRENGTH_CHARGE_TIME, "s")
		return
	# Incite below 80 influence
	if influence >= BERSERK_THRESHOLD or incite_cooldown > 0 or not _can_gain_influence():
		return
	var cost = player.hp * INCITE_HP_COST
	if cost <= 0:
		return
	incite_cooldown = _get_cooldown(INCITE_COOLDOWN)
	var old_hp = player.hp
	player.hp = max(player.hp - cost, 0.0)
	_apply_influence(cost, old_hp)
	var lost_hp = player.max_hp - player.hp
	influence = min(influence + INCITE_INFLUENCE_FLAT + lost_hp * INCITE_INFLUENCE_PCT, MAX_INFLUENCE)
	_reset_passive_timer()
	print("Tyrant: Incite — Influence spiked")
